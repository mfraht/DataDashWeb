...
#df = get booking details, df1 = booking details with customers
df = db.getBookingDetails()
df = df['Year']=df['TripStart'].dt.year
df = df.groupby(["Year"]).sum()[['BasePrice', 'AgencyCommission']]
# df1 = db.getBookings()
# df2 = db.getCustomers()
...
#Merge bookings and customers collection ==df3
# bookings_with_customers = pd.merge(df1, df2, on="BookingId")
